...
class CommentsSlider extends GlobalSlider {
    constructor(data, options) {
        super(data, options);
        this.resetAnimationOnSlider(data, options)
    };
    resetAnimationOnSlider(data, options) {
        const prev = document.querySelector(data.prev);
        const next = document.querySelector(data.next);
        const slide = document.querySelector(data.slide);
        const reset = (button) => {
            return button.addEventListener("click", () => {
                if (options.resetAnimationOnSlider) return slide.style.transition = "none";
                return false;
            });
        };
        reset(prev);
...
const blogSlider = new BlogSlider(data.otherVariables.blog.blogSlider, {
    value_1: 100,
    breakpoint: -200,
    unit: "%"
});
const commentsSlider = new CommentsSlider(data.otherVariables.blog.commentsSlider, {
    value_1: 100,
    breakpoint: -200,
    unit: "%",
    resetAnimationOnSlider: true
});